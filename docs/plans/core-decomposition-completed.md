...
- `services-core` 已承接 session storage file layout、目录创建 helper、turn file indexing/deletion、metadata pagination、metadata counter/index mutation 规则和通用 JSON file store；core persistence manager 保留 workspace-to-sessions-root 解析、metadata IO/update orchestration 和产品 runtime 接线。
