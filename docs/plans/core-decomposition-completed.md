...
- `services-core` 已承接 session storage file layout、目录创建 helper、turn file indexing/deletion、metadata pagination、metadata construction/counter/index/field mutation、lineage / branch metadata shaping 规则和通用 JSON file store；core persistence manager 保留 workspace-to-sessions-root 解析、metadata IO orchestration 和产品 runtime 接线。
- `services-integrations` 已承接 remote-connect pairing/encryption/QR/relay client primitives、wire command routing、response assembly、workspace/session/poll/file/dialog/cancel/interaction helper 和 state tracker contract；core remote-connect root 只保留旧路径 compatibility re-export，core `RemoteServer` 保留 product host adapter、initial sync host glue、全局 tracker adapter 与 concrete runtime host 接线。
...
- `tool-execution`（Cargo package `tool-runtime`）已承接本地 Write / Edit / Delete / Glob、远程 Delete / Read / LS / Glob / Grep、tool pipeline batching plan、retry policy、state counting、cancellation state policy，以及 Bash shell 可复用策略：禁用命令、工作目录命令包装、非交互环境、AppleScript/IM guard、本地/远程结果渲染和 background result 文本。core BashTool / ToolPipeline 只保留 agent-facing adapter、终端 session、权限 UI/channel、checkpoint 采集、cancellation token/channel、scheduler delivery 和 host context glue。
...
- `tool-contracts` 已承接 tool pipeline 的截断恢复提示和 write-like tool 分类；`tool-execution` 已承接 state counting 和 cancellation state policy；`agent-runtime` 已承接 tool confirmation plan/failure/wait-result 与 light checkpoint summary policy；core tool pipeline / tool context 只委托这些纯策略并保留执行状态存储、permission channel、Git facts 采集、scheduler 和 cancellation token glue。
...
- `agent-runtime` 已承接 scheduler/background delivery 纯决策、turn outcome lifecycle plan、thread goal runtime、subagent visibility、prompt cache facts、prompt environment facts、mode/source presentation facts、scheduled-job lifecycle state、custom subagent schema/default/markdown IO/discovery/loading、post-call hook routing、tool confirmation plan、goal/user-question tool wire contract、SessionControl 输入契约、remote session create host、remote dialog / SessionMessage / scheduled-job queued-turn lifecycle façade、SessionMessage / CronTool target session resolution/list validation、background result / thread-goal delivery / remote cancel host、SessionControl create/list/delete/workspace-resolution/cancel façade、部分 event fact 映射、DeepResearch citation renumber 纯重排逻辑，以及 DeepReview policy / manifest / budget / queue / report enrichment / incremental cache / shared-context runtime state / task-execution provider-neutral packet、retry、backoff、capacity-skip shaping、provider capacity error decision、provider/admission queue step decision、provider/reviewer queue wait timing 状态机、provider retry attempt/elapsed 状态机、runtime tracker reliability signal shaping、runtime diagnostics log shaping 和 TaskTool retry/cancelled/result presentation。
- core 仍保留 concrete session manager、metadata IO orchestration、scheduler lifecycle、event emitter、permission UI/channel wait、concrete prompt assembly 主体、DeepReview task launch / provider wait side effect / report persistence、product `Tool` adapter 和具体 hook side effect；remote dialog submit、SessionMessage target resolution/list/submit、CronTool target resolution/list validation、scheduled-job submit、Bash / subagent background result delivery、thread-goal delivery、SessionControl create/list/delete/workspace resolution 和 requester-aware cancellation 已改为经 `AgentRuntime` lifecycle / session-management / cancellation port 进入 core owner adapter，保留 queue/policy/reply route/image/prepended reminder、background injection/follow-up、thread-goal resume/update、session lookup/delete 可见结果和取消回复抑制语义；session storage file layout / turn file indexing-deletion / metadata pagination / metadata construction/counter-index/field mutation / lineage / branch metadata shaping rules / JSON file IO 已委托 `services-core`，DeepResearch report 文件 IO / post-turn hook 已委托 `services-integrations`。
...
- boundary check 覆盖 remote-connect pairing/encryption/QR/relay primitive、command routing 和 response assembly owner，要求 core 委托 `services-integrations`，并阻止 primitive implementation、response assembly / command policy 回流 core。
...
- boundary check 覆盖 tool truncation recovery presentation、confirmation wait-result、light checkpoint summary、batching plan、retry policy、DeepReview task-execution 纯策略、provider capacity error decision、provider/admission queue step decision、provider retry attempt/elapsed 状态机、TaskTool retry/cancelled/result presentation、DeepResearch citation renumber 纯重排 owner，以及 MiniApp manager workflow、compile request/path adaptation、permission policy request/path adaptation 和 import lifecycle orchestration owner，要求 core tool pipeline / tool context / DeepReview task adapter / DeepResearch citation hook / MiniApp manager 委托 `bitfun-agent-tools`、`bitfun-agent-runtime`、`tool-runtime::pipeline` 和 `bitfun-product-domains`。
- boundary check 覆盖 AgentRuntime dialog lifecycle / delivery / session-management / cancellation ports、remote image attachment adapter、portable prepended reminder adapter、remote dialog host submit、remote cancel host、SessionMessage submit / target session lookup、CronTool target session lookup、scheduled-job submit、background result delivery、thread-goal delivery 和 SessionControl create/list/delete/workspace-resolution/cancel 路径，阻止这些入口重新直接绕过 `AgentRuntime` 调用 core scheduler 或 coordinator。
...
- concrete session manager、scheduler concrete lifecycle、event delivery、permission UI/channel wait、prompt assembly、session metadata IO orchestration、AI client factory / provider acquisition 仍在 core。
- Bash tool orchestration 的可复用 shell helper、本地 indexed workspace search owner、session storage file layout / turn file indexing-deletion / metadata pagination / metadata construction/counter-index/field mutation / lineage / branch metadata shaping rules / JSON file IO、remote workspace search concrete owner、remote SSH/SFTP/PTY concrete owner、tool confirmation/checkpoint 纯策略、tool pipeline batching/retry/state-count/cancellation-state policy、DeepReview provider-neutral policy/report/cache/task-execution shaping、provider capacity error decision、provider/admission queue step decision、provider/reviewer queue wait timing 状态机、provider retry attempt/elapsed 状态机、runtime tracker reliability signal shaping、runtime diagnostics log shaping、TaskTool retry/cancelled/result presentation、DeepResearch citation renumber 纯重排 / report IO、MiniApp host dispatch、MiniApp built-in seed/marker IO、MiniApp JS worker process/pool lifecycle、MiniApp storage filesystem IO、MiniApp import bundle IO、MiniApp import bundle planning、MiniApp compile request/path adaptation、MiniApp permission policy request/path adaptation、MiniApp import lifecycle orchestration、MiniApp manager create/update/draft/apply/customization workflow 持久化顺序和 prompt environment facts 已迁出；permission UI/channel side effect、tool pipeline concrete token/channel/scheduler glue、DeepReview concrete launch/provider wait side effect/report persistence，以及 MiniApp AI acquisition 和更大 concrete workflow execution 仍未完成 owner 迁移。
