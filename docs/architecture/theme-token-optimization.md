...
| 区域 | 当前出现次数 | 当前唯一色数 | 说明 |
| --- | ---: | ---: | --- |
| Theme presets | 228 | 203 | 主题个性与 palette 映射，不作为普通 app literal 直接合并；仅在同一主题内角色可证明等价时由 preset helper 复用 |
...
| Theme runtime | 40 | 40 | `ThemeService.ts` 运行时注入；暗色 card alpha fallback 已收敛到现有 overlay token |
...
| --- | ---: | ---: | --- |
| Theme presets | 1 | 185 | 优先处理同一 theme 内 hex/rgb 完全等价和同 alpha 阶重复；保留主题个性 |
| Theme runtime | 0 | 28 | 先与静态 token 和 payload contract 对齐，避免 early render 或 system theme 回退 |
...
| `src/web-ui/src/infrastructure/theme/core/ThemeService.ts` | 40 | 运行时注入；需保持 early render、system theme 和 payload 导出兼容 |
...
| `colorDomainNearPairs.nearTotal` | 346 | 346 | 控制 theme preset/runtime/token/editor/Mermaid 等专用域 near 队列规模 |
...
2. 自定义主题扩展后续体验优化：custom theme 校验、加载、注册、导出和 preview 输入已绑定到 TS schema；
   如需继续改善首屏体验，只允许由 TS schema 生成最小 bootstrap cache，不允许 Rust 直接拥有 custom theme schema。
3. generated widget 兼容面维护：payload 已停止导出 `background/bg/text/radius/spacing` legacy key；
