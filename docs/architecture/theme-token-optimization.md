...
`450` 个 web-ui 唯一颜色是前端生产文件的全域审计数，不是普通 app UI 的色值预算。
...
| 专用域需证据复核的 near color pair | 109 |

跨 root 基线必须分别看待；这些数字不是互相累加后的单一上限：

| root | 颜色出现次数 | 唯一颜色 | App UI raw | fallback var | unresolved / non-contract key | dynamic family | 说明 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| `src/web-ui/src` | 658 | 450 | 0 | 0 | 0 | 13 | 主应用完整主题、专用 palette、widget payload 和 editor/terminal/Mermaid 域。 |
| `src/mobile-web/src` | 36 | 31 | 1 | 0 | 0 | 3 | 仅保留图片缩略图黑色 overlay 上的 `#fff` 关闭按钮前景；现有 token 无法同时保证亮暗主题安全。 |
| `BitFun-Installer/src` | 203 | 145 | 0 | 0 | 0 | 2 | 安装器主题数据和静态变量纳入 contract；runtime 只导出实际消费的 accent/purple 色阶。 |
...
| 区域 | 当前出现次数 | 当前唯一色数 | 说明 |
| --- | ---: | ---: | --- |
| Theme presets | 186 | 155 | 主题个性与 palette 映射，不作为普通 app literal 直接合并；仅在同一主题内角色可证明等价时由 preset helper 复用 |
...
| --- | ---: | ---: | --- |
| Theme presets | 0 | 80 | 优先处理同一 theme 内 hex/rgb 完全等价和同 alpha 阶重复；保留主题个性 |
...
| installer dark runtime panel `#18181a` -> `#1a1c1e` | merge | `BitFun-Installer/src/theme/installerThemesData.ts` | 仅调整 runtime preset，复用已有 Slate panel 值降低 theme preset near pair；静态 `variables.css` 背景保持原值，避免 token-contract near pair 增长 |
...
| web/installer cyber deep blacks | merge | `src/web-ui/src/infrastructure/theme/presets/cyber-theme.ts`、`BitFun-Installer/src/theme/installerThemesData.ts` | `#101010`、`#0d0d0d` 和旧 tooltip `rgba(16, 16, 16, 0.95)` 收敛到既有 dark primary `#0e0e10` 派生值；RGB distance <= 3.32，主题识别仍由 neon accent、surface 和 typography 表达；安装器同名主题预览同步避免跨 surface 漂移 |
...
| `colorDomainNearPairs.nearTotal` | 109 | 109 | 控制 theme preset/runtime/token/editor/Mermaid 等专用域 near 队列规模 |
...
1. 专用域近似色复核：mobile-web theme preset near pair 已清零；web-ui/installer 已先处理不可感知的跨主题 neutral 和 runtime panel 值。
