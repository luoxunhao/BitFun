...
- 第三方模块 import 前先加载已保存的全局/来源/target 策略和本次安全启动参数。默认兼容模式直接继续，不增加
...
| 新增/删除工具、Hook、权限或依赖 | 候选激活前显示简短差异、来源、target 和作用域；默认 5 秒非阻塞切换窗口后按兼容策略继续。期间的暂停/受限/停用先使候选失效，再重新评估；保留一次持久摘要。 |
...
| worker 崩溃 | 在途调用以 `worker-lost` 失败且不自动重放；只有精确旧物化目录仍可校验时才能重建旧代次，否则显示“上一版本不可恢复”；默认 10 分钟内按 1、5、30 秒退避重启 3 次。 |
