...
- `relay-service` 已承接 room/device 状态、account/sync 存储、asset store 和 HTTP/WebSocket router；standalone app 保留 bind、环境配置、静态 fallback、进程生命周期和管理 CLI，embedded 入口复用同一 router。embedded 的 bind、静态 fallback 和任务生命周期仍在 assembly 兼容路径，尚未完成宿主归位。
