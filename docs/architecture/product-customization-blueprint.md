...
本设计替换旧术语，但不丢失其有效完整性保证：

| 现有对象 | 目标对象 | 必须保留的迁移保证 |
|---|---|---|
| Product Profile | 产品定义 | 产品 ID、数据命名空间、能力上限、发行和更新事实一一迁移；运行时不读取作者源文件 |
| Brand Pack | 品牌资源 | 路径边界、平台变体和内容摘要进入构建校验，不变成可执行包 |
| GUI/TUI Surface Blueprint | GUI/TUI 布局选择 + 宿主注册表 | 只迁移已注册 ID；GUI/TUI 仍独立校验，不共享 schema；最终引用及摘要进入组装结果 |
| Resolved Product Manifest | 产品组装结果 | 保留不可变内容摘要、打包/签名绑定和各消费方一致性；迁移期可生成旧只读格式供现有代码读取 |

迁移顺序是“新校验器同时生成产品组装结果和旧只读格式 → 主应用/CLI/安装器/updater 逐一切换 → 删除旧作者对象
读取”。在所有消费方切换前，不得让两套输入各自决定产品身份、更新渠道、签名公钥或内置扩展版本。
...
产品定义只决定能力上限和默认值，不记录动态服务健康或插件故障。运行时状态统一显示为可用、准备中、部分可用、
策略限制、不支持或失败，并给出原因。隐藏导航或面板不表示后端能力已禁用；需要安全裁剪时必须同时约束后端
注册、入口配置和插件贡献。
...
| 启用 | 由产品定义选择 | 保留现有来源确认和激活 | 按 OpenCode 配置与标准目录自动加载；策略可停用，不二次审批 |
| 更新 | 随产品升级和回滚 | 用户或组织独立更新、停用和卸载 | 候选版本准备、差异提示；失败时只保留健康旧进程，精确旧物化目录可校验时才重建 |
| 权限 | 使用同一有效策略；直接脚本副作用受真实 OS/容器边界限制 | 同左 | 同左 |
| 执行 | 与其他插件走同一进程隔离、期限、取消和恢复路径 | 同左 | 同左 |
| 冲突 | 普通贡献遵循目标生态顺序；少量产品保护项除外 | 按对应 BitFun 规则 | 按 OpenCode 顺序，可覆盖普通内置贡献 |
