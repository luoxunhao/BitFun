...
- Rust/desktop 侧由 TS 主题预设生成的最小首屏字段文件。
...
产品定制中的品牌资源和 GUI/TUI 布局选择只引用对应界面宿主已注册的 theme/preset ID 或明确列出的语义色；
详细定制边界见
[`product-customization-blueprint.md`](product-customization-blueprint.md)。布局选择不拥有主题 schema，构建
任务也不能生成未登记的 CSS variable、GUI token、TUI palette 或跨端通用主题 key 来绕过本文件审计。

- [OpenCode theme](https://opencode.ai/docs/themes/) 采用可扩展 JSON schema、内置/用户/项目多层加载、
  可复用色值定义和 light/dark variant；这是一套 TUI 主题格式，不能用 BitFun GUI 的七个语义色替代。
...
- [Codex config](https://developers.openai.com/codex/config-basic) 使用分层 config 和明确优先级；
  对 BitFun 的启发是：Rust/host 可以读取偏好与可信配置层，但不应成为 UI token 的定义方。

BitFun 自身的 GUI 插件色彩接口目前只提供 `primary`、`secondary`、`accent`、`success`、`warning`、`error`、
`info` 七个语义色，入口是 `src/web-ui/src/infrastructure/theme/pluginThemeProjection.ts`。这七个字段只服务
BitFun GUI 插件，不代表 OpenCode theme schema，也不代表 OpenCode TUI `api.theme` 已完成兼容。

OpenCode 主题兼容由 CLI/TUI 适配器独立处理：按 OpenCode 的内置、用户、项目、当前目录顺序加载主题 JSON，
保留稳定字段、颜色引用和 light/dark 变体，再映射到 Ratatui/终端颜色能力。缺失终端颜色能力时按 ANSI 或
默认终端色降级。GUI 不读取这套原始字段；若未来 GUI 插件确实需要 surface/text/border 等语义，应先增加
有真实消费方的小型 GUI 字段集合，而不是暴露完整 `ThemeConfig` 或全部 CSS 变量。
...
| 首屏 bootstrap | 只注入 WebView 首屏所需字段：`data-theme`、`data-theme-type` 和核心背景/文本 CSS 变量。 | JS 启动后必须重新应用完整主题，覆盖 bootstrap 临时值并恢复 Monaco、Mermaid、terminal、widget payload 等专用域。 |
| 生成式 UI 主题提示 | 只读取 TS 预设生成的模型提示主题文件；不得手写内置主题 palette。 | 定义生成规则；新内置主题加入 `builtinThemes` 后由生成器同步到 Rust 只读文件。 |
| BitFun GUI 插件主题色 | 不维护插件色彩 schema，不把插件颜色解析放到 Rust/desktop。 | 由 `pluginThemeProjection.ts` 从完整主题生成七个语义色；插件不得直接依赖内部 CSS var 全量表。 |
| OpenCode TUI 主题 | CLI/Rust 只解释 OpenCode 稳定主题字段并映射终端颜色，不读取 GUI `ThemeConfig`。 | 不承担 OpenCode TUI 主题来源或覆盖顺序；GUI 与 TUI 主题保持独立。 |
| 内置主题扩展 | 不手写新增完整 palette。只有新内置主题需要首屏无闪烁时，才更新最小 bootstrap 字段。 | 新主题先进入 TS 预设和 `builtinThemes`；所有语义、组件和专用域 token 以 TS 侧为准。 |
...
| CSS 变量和 key 命名 | 只允许新增明确写入首屏主题文件的启动字段；不得新增 backend theme service 或平行 alias 表。 | 新 primitive/semantic/component key、兼容 alias、surface rename、dynamic family 和 widget payload key 均在 TS contract/audit 中登记。 |
| 专用渲染域 | Core/web host 不维护 Monaco、Mermaid、Prism、language identity、UI exception 等 web-ui 色板；CLI/TUI preset 和 terminal ANSI 是独立终端 surface，不能成为 web-ui 主题源头。 | web-ui 专用域由对应 TS 模块维护，按 `colorDomainScopes.*` 和 `colorDomainNearPairs.*` 预算治理。 |
...
登记归属模块，审计脚本按根目录归属判断失效的动态字段族，避免 web-ui、mobile-web 和
installer 互相误报或重复定义。

CLI/TUI 是独立终端产品 surface，不属于 web-ui 主题源，也不由 desktop/backend 定义主题语义。
`src/apps/cli/themes/presets/*.json` 拥有 CLI preset 数据，`src/apps/cli/src/ui/theme.rs`
只负责 ANSI/monochrome 降级适配、OpenCode-compatible preset JSON 的解析和运行时颜色映射，
不拥有 web-ui `ThemeConfig`。跨 web-ui、installer、CLI 的近似色可以在语义一致且不会影响相邻状态区分时手动收敛，
但不能通过让 Rust/CLI 复制 web-ui `ThemeConfig` 来解决；若后续需要共享，只能新增明确的 TS 生成字段集或 CLI schema
contract，并纳入审计基线。

当前 Rust 首屏主题已改为读取 TS 内置主题预设生成的
...
避免 Rust 再手写一套 web-ui 主题色。这两个生成文件都只读：startup 文件面向 WebView 首屏，模型提示文件面向
生成式 UI，均不能反向成为主题源头。
...
3. 只有影响 JS 加载前首屏的字段，才更新 Rust bootstrap 文件；不能为了 Rust 读取方便新增第二套主题 schema。
...
和 CSS var 注入都必须由 TS 侧 resolver/helper 统一生成；Rust/desktop 只消费 TS 生成的最小字段集，不复制
这些派生规则。
...
模型提示主题文件；mobile-web 和 installer 颜色在各自 TS root 中维护；CLI/TUI
...
| 区域 | 当前出现次数 | 当前唯一色数 | 说明 |
| --- | ---: | ---: | --- |
| Theme presets | 159 | 112 | 主题个性与 palette 映射；跨主题深色 neutral、弱文本、非状态浅色背景和同概念 success 色已收敛；相邻 surface、主题识别主背景、状态色和 editor lineHighlight 继续保留；不再对外输出的 accent/purple/background/element authoring stop 已从 preset schema 移除 |
| Token contracts | 80 | 72 | `tokens.scss` 等静态契约根；git/diff 派生背景和 hover 不再作为主题输入或 root/runtime contract，消费侧从 git 语义文本色局部派生；黑白 overlay alpha stop 继续保留相邻状态层级，未消费 legacy mixin、自引用别名、死 Sass helper、低复用单 surface helper 和不再输出的 root stop 已移除，不按数值相近强行合并 |
...
| `generatedWidgetPayload.varUnique` | 56 | 56 | 控制 widget 对外主题 payload allowlist 不继续膨胀；该预算只覆盖主题敏感 canonical 子集和 button component 字段，宿主内部、shape/spacing/font-size/font-weight、静态 overlay、派生 key 和历史兼容 key 不计入 payload API |
...
- `src/apps/desktop/src/theme.rs` 只维护 WebView 首屏最小 bootstrap 字段，不能扩展为完整主题 schema。
...
2. 插件/主题扩展治理：BitFun GUI 插件只消费 `pluginThemeProjection.ts` 暴露的七个语义色；这不是 OpenCode
   theme schema。新增 GUI 插件需求必须先证明无法复用现有字段，再增加有真实消费方的小型字段集。不得把
   generated widget payload 或完整 runtime CSS var 表作为插件主题 schema。OpenCode TUI 主题由 CLI 适配器按
   官方格式独立解析和映射。
...
4. CLI/TUI palette 压缩：CLI/TUI 已纳入 `theme:color-audit:cli`，truecolor fallback 由内置 preset JSON 派生，Rust 不再手写独立 RGB palette；Rust fallback、preset 和 runtime-consumed preset near pair 均为 0。后续只处理 preset 内能证明同一终端 surface 语义且已被 CLI renderer 消费的近似色，
   不能让 Rust/CLI 复制 web-ui palette。若要跨 surface 共享，只能先定义共享语义字段或生成链路。
...
- generated widget payload 不导出宿主 FlowChat、navigation、z-index、shape/spacing/font-size/font-weight 和 tool-card 内部 key；iframe 内容保留独立布局和
  stacking context，只通过核心 canonical color/text/surface/status/border/element/shadow、motion/font family 和 `components.button` 字段获取必要主题信息，shape/spacing/typography 默认与 tool-card 所需派生值由 iframe fallback/static shell 提供。Canvas iframe 复用同一 payload，但 shape/spacing/typography 由 Canvas runtime fallback 提供；custom theme 若主要依赖密度、圆角、字号或字重建立风格，iframe 内不会完整同步这些风格，这是为了保持对外字段上限的有意取舍。
...
- ThemeService 的 runtime dynamic family 采用显式白名单导出，包括 accent、purple、shadow、
  blur、受限的 radius/spacing、motion、easing、受限的 font weight/font size 和 line-height；radius 只输出到 `xl`，
  spacing 只输出到 `8`，`2xl/full/10/12/16` 保留在 TS schema 和 iframe fallback 兼容面，不再是 Web root runtime contract。custom theme
