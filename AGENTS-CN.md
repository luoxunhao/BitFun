...
法律保护项 ID；不得承载用户/来源级插件策略、安装、激活、更新、权限或动态健康状态。迁移期间可以保留现有
Product Profile、Surface Blueprint 和 Resolved Product Manifest 代码，但新增设计与实现必须遵循产品定制
设计中的目标对象和迁移规则。
...
在对应 OC-R 阶段实现并通过验证前，当前 P0 适配器仍只是静态预览/导入代码。不得继续把旧受管包路径扩张为
