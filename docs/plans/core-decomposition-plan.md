...
| M12 Product shape / SDK readiness / Core facade 收口 | 基于 capability matrix 裁剪 no-default/product-full 和不同交付形态依赖；建立内部 Agent Runtime SDK façade 的最小可嵌入路径；将远程、bot、产品工具等会话创建/取消入口迁入 port-backed `AgentRuntime`；将 `bitfun-core` 限定为 compatibility facade、product-full assembly 与少量迁移期 adapter。scheduler 队列、reply route、事件副作用和 session metadata/update orchestration 未具备等价 port 前不得硬迁 | cargo metadata / cargo tree 有对比数据；Agent SDK façade 可用 fake provider 跑通最小 session / turn / event stream；各产品形态关键入口验证通过；边界脚本阻止 owner 逻辑回流 |
